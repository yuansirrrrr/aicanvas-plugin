...
def _log_subscription_gate_event(event, **context):
    safe_context = {}
    for key, value in context.items():
        if re.search(
            r"api[-_ ]?key|token|authorization|password|passwd|pwd|cdkey|secret|cookie|session|bearer",
            str(key or ""),
            flags=re.IGNORECASE,
        ):
...
def _summarize_runninghub_field_value(value):
    raw = str(value or "").strip()
    if not raw:
        return {"hasValue": False, "kind": "empty", "preview": ""}
    if raw.startswith("data:"):
        return {"hasValue": True, "kind": "data-url", "preview": raw[:48] + "..."}
    if raw.startswith("http://") or raw.startswith("https://"):
        try:
            ...
            return {
                "hasValue": True,
                "kind": "url",
                "preview": f"{parsed.scheme}://{parsed.netloc}/{basename}" if basename else f"{parsed.scheme}://{parsed.netloc}",
            }
        except Exception:
            return {"hasValue": True, "kind": "url", "preview": raw[:120]}
    return {
        "hasValue": True,
        "kind": "local-or-raw",
        "preview": os.path.basename(raw) or raw[:120],
    }


def _summarize_runninghub_node_info_list(node_info_list):
    ...
    for item in node_info_list:
        if not isinstance(item, dict):
            continue
        value_summary = _summarize_runninghub_field_value(item.get("fieldValue"))
        summary.append(
            {
...
def _log_runninghub_workflow_event(event, **context):
    safe_context = {}
    for key, value in context.items():
        if re.search(
            r"api[-_ ]?key|token|authorization|password|passwd|pwd|cdkey|secret|cookie|session|bearer",
...
def _extract_node_info_list_from_runninghub_demo(demo_text):
    text = str(demo_text or "")
    marker = '"nodeInfoList"'
    marker_index = text.find(marker)
    if marker_index < 0:
        marker_index = text.find("'nodeInfoList'")
    if marker_index < 0:
        return []
    ...
    if list_start < 0:
        return []
    try:
        value, _ = json.JSONDecoder().raw_decode(text[list_start:])
        return value if isinstance(value, list) else []
    except Exception:
        return []


def _fetch_runninghub_ai_app_demo_nodes(api_key, webapp_id):
    key = str(api_key or "").strip()
    app_id = str(webapp_id or "").strip()
    if not key or not app_id:
        return []
    demo_url = (
        "https://www.runninghub.cn/api/webapp/apiCallDemo?"
        + urllib.parse.urlencode({"apiKey": key, "webappId": app_id})
    )
    headers = {"Authorization": f"Bearer {key}", "User-Agent": "Mozilla/5.0"}
    try:
        import requests as _req
        resp = _req.get(demo_url, headers=headers, timeout=60)
        resp.raise_for_status()
        payload = resp.json()
    except ImportError:
        req = urllib.request.Request(demo_url, headers=headers, method="GET")
        with urllib.request.urlopen(req, timeout=60) as resp:
            payload = json.loads(resp.read().decode("utf-8", errors="replace"))
    data = payload.get("data") if isinstance(payload, dict) else {}
    curl_text = ""
    if isinstance(data, dict):
        curl_text = data.get("curl") or data.get("python") or data.get("javascript") or ""
    elif isinstance(data, str):
        curl_text = data
    return _extract_node_info_list_from_runninghub_demo(curl_text)


def _log_runninghub_ai_app_demo_nodes(api_key, workflow_id):
    try:
        nodes = _fetch_runninghub_ai_app_demo_nodes(api_key, workflow_id)
        _log_runninghub_workflow_event(
            "ai_app_demo_nodes",
            workflowId=workflow_id,
            nodeCount=len(nodes),
            nodes=_summarize_runninghub_node_info_list(nodes),
        )
    except Exception as exc:
        _log_runninghub_workflow_event(
            "ai_app_demo_nodes_failed",
            workflowId=workflow_id,
            error=repr(exc),
        )
...
def _enforce_vip_subscription_gate(handler, payload=None, required_model_id=""):
    ...
    _log_subscription_gate_event(
        "subscription_gate_decision",
        path=getattr(handler, "path", ""),
        requiredModelId=required_model_id,
        resolvedRequiredModelId=decision.get("requiredModelId") if isinstance(decision, dict) else "",
        ...
        reasonCode=decision.get("reasonCode") if isinstance(decision, dict) else "",
    )
...
OPENCODE_CANVAS_API_PREFIX = "/api/v2/opencode-canvas"
OPENCODE_CANVAS_COMMAND_TIMEOUT_SECONDS = _get_int_env("AICANVAS_COMMAND_TIMEOUT_SECONDS", 30, 1)
OPENCODE_CANVAS_IMAGE_GENERATION_TIMEOUT_SECONDS = _get_int_env("AICANVAS_IMAGE_GENERATION_TIMEOUT_SECONDS", 180, 1)
OPENCODE_CANVAS_VIDEO_GENERATION_TIMEOUT_SECONDS = _get_int_env("AICANVAS_VIDEO_GENERATION_TIMEOUT_SECONDS", 480, 1)
OPENCODE_CANVAS_GENERATION_TIMEOUT_SECONDS = _get_int_env(
    "AICANVAS_GENERATION_TIMEOUT_SECONDS",
...
_OPENCODE_CANVAS_STATIC_SKILL_CACHE = {
    "signature": "",
    "skills": [],
}


def _opencode_canvas_now_ms():
    return int(time.time() * 1000)


def _opencode_canvas_parse_json(handler, max_bytes=1024 * 1024):
    body = _read_body(handler, max_bytes=max_bytes)
    if not body:
        return {}
    return json.loads(body.decode("utf-8", errors="replace"))


def _opencode_canvas_load_static_agent_skills():
    skills_dir = os.path.join(CURRENT_DIR, "src", "modules", "agent", "skills")
    try:
        entries = [
    ...
    try:
        signature = "|".join(
            f"{os.path.basename(path)}:{os.path.getmtime(path)}:{os.path.getsize(path)}"
            for path in entries
        )
    except OSError:
        signature = ""

    cached = _OPENCODE_CANVAS_STATIC_SKILL_CACHE
    if signature and cached.get("signature") == signature:
        return list(cached.get("skills") or [])
...
def _opencode_canvas_merge_agent_skills(live_skills):
    merged = []
    seen = set()
    for skill in live_skills if isinstance(live_skills, list) else []:
        ...
        seen.add(skill_id)
        merged.append(skill)
    for skill in _opencode_canvas_load_static_agent_skills():
        skill_id = str(skill.get("id") or "").strip()
        if not skill_id or skill_id in seen:
            continue
        seen.add(skill_id)
...
def _opencode_canvas_update_runtime(payload):
    ...
    with _OPENCODE_CANVAS_CONDITION:
        ...
        if isinstance(payload.get("skills"), list):
            _OPENCODE_CANVAS_STATE["skills"] = payload.get("skills")
...
def _opencode_canvas_runtime_poll(payload):
    _opencode_canvas_update_runtime(payload)
    with _OPENCODE_CANVAS_CONDITION:
        pending = list(_OPENCODE_CANVAS_STATE.get("pending") or [])
        _OPENCODE_CANVAS_STATE["pending"] = []
        return {
            "ok": True,
            "jobs": pending,
            "serverTime": _opencode_canvas_now_ms(),
        }


def _opencode_canvas_runtime_result(payload):
    payload = payload if isinstance(payload, dict) else {}
    ...
    result = {
        ...
        "completedAt": _opencode_canvas_now_ms(),
    }
    with _OPENCODE_CANVAS_CONDITION:
        _OPENCODE_CANVAS_STATE["results"][job_id] = result
        _OPENCODE_CANVAS_CONDITION.notify_all()
    return {"ok": True, "jobId": job_id}


def _opencode_canvas_iter_context_nodes(value):
    if isinstance(value, dict):
        for key in ("nodes", "selectedNodes"):
            nodes = value.get(key)
        ...
        for child in value.values():
            if isinstance(child, (dict, list)):
                yield from _opencode_canvas_iter_context_nodes(child)
...
def _opencode_canvas_generation_kind(args):
    args = args if isinstance(args, dict) else {}
    for key in ("generationType", "nodeType", "type", "kind"):
    ...
    node_id = str(args.get("nodeId") or args.get("id") or "").strip()
    if node_id:
        with _OPENCODE_CANVAS_CONDITION:
            context = dict(_OPENCODE_CANVAS_STATE.get("context") or {})
        for node in _opencode_canvas_iter_context_nodes(context):
            if not isinstance(node, dict):
                continue
            candidate_id = str(node.get("id") or node.get("nodeId") or "").strip()
            if candidate_id != node_id:
                continue
            node_type = str(node.get("type") or node.get("nodeType") or node.get("kind") or "").lower()
...
def _opencode_canvas_command_timeout_seconds(command_id, args):
    if command_id != "generation.run":
        return OPENCODE_CANVAS_COMMAND_TIMEOUT_SECONDS
    generation_kind = _opencode_canvas_generation_kind(args)
    if generation_kind == "image":
        return OPENCODE_CANVAS_IMAGE_GENERATION_TIMEOUT_SECONDS
    if generation_kind == "video":
        return OPENCODE_CANVAS_VIDEO_GENERATION_TIMEOUT_SECONDS
    return OPENCODE_CANVAS_GENERATION_TIMEOUT_SECONDS


def _opencode_canvas_submit_command(command_id, args=None, timeout_seconds=OPENCODE_CANVAS_COMMAND_TIMEOUT_SECONDS):
    command_id = str(command_id or "").strip()
    if not command_id:
        return 400, {"ok": False, "error": "Missing command id"}
    args = args if isinstance(args, dict) else {}
    ...
    with _OPENCODE_CANVAS_CONDITION:
        ...
        _OPENCODE_CANVAS_STATE["pending"].append({
            ...
            "args": args,
            "createdAt": _opencode_canvas_now_ms(),
        })
        _OPENCODE_CANVAS_CONDITION.notify_all()

        while job_id not in _OPENCODE_CANVAS_STATE["results"]:
            remaining = deadline - time.time()
            if remaining <= 0:
                _OPENCODE_CANVAS_STATE["pending"] = [
                    job for job in _OPENCODE_CANVAS_STATE.get("pending") or []
                    if job.get("jobId") != job_id
                ]
                return 504, {"ok": False, "jobId": job_id, "error": "Canvas command timed out"}
            _OPENCODE_CANVAS_CONDITION.wait(timeout=min(remaining, 1.0))

        result = _OPENCODE_CANVAS_STATE["results"].pop(job_id)
    ...
    return 500, {
        ...
        "jobId": job_id,
        "command": command_id,
        "error": result.get("error") or "Canvas command failed",
    }


def _opencode_canvas_json_response(handler, status_code, payload):
    if status_code == 200:
...
def _opencode_canvas_handle_get(handler, path):
    if path == OPENCODE_CANVAS_API_PREFIX + "/health":
        _json_ok(handler, _opencode_canvas_public_state())
        return True
    if path == OPENCODE_CANVAS_API_PREFIX + "/commands":
        with _OPENCODE_CANVAS_CONDITION:
            _json_ok(handler, {"ok": True, "commands": _OPENCODE_CANVAS_STATE.get("commands") or []})
        return True
    if path == OPENCODE_CANVAS_API_PREFIX + "/skills":
        with _OPENCODE_CANVAS_CONDITION:
            skills = _opencode_canvas_merge_agent_skills(_OPENCODE_CANVAS_STATE.get("skills") or [])
            _json_ok(handler, {"ok": True, "skills": skills})
        return True
    if path == OPENCODE_CANVAS_API_PREFIX + "/context":
        with _OPENCODE_CANVAS_CONDITION:
            skills = _opencode_canvas_merge_agent_skills(_OPENCODE_CANVAS_STATE.get("skills") or [])
            _json_ok(handler, {
                "ok": True,
                "context": _OPENCODE_CANVAS_STATE.get("context") or {},
                "runtime": _opencode_canvas_public_state(),
                "commands": _OPENCODE_CANVAS_STATE.get("commands") or [],
                "skills": skills,
            })
        return True
    return False
...
def _opencode_canvas_command_from_post(path, payload):
    prefix = OPENCODE_CANVAS_API_PREFIX
    if path.startswith(prefix + "/commands/"):
        command_id = urllib.parse.unquote(path[len(prefix + "/commands/"):])
        args = payload.get("args") if isinstance(payload, dict) and isinstance(payload.get("args"), dict) else payload
        return command_id, args if isinstance(args, dict) else {}
    if path == prefix + "/nodes":
        return "node.create", payload
    if path.startswith(prefix + "/nodes/") and path.endswith("/prompt"):
        node_id = urllib.parse.unquote(path[len(prefix + "/nodes/"):-len("/prompt")])
        args = payload if isinstance(payload, dict) else {}
        return "node.setPrompt", {"nodeId": node_id, **args}
    if path.startswith(prefix + "/nodes/") and path.endswith("/params"):
        node_id = urllib.parse.unquote(path[len(prefix + "/nodes/"):-len("/params")])
        args = payload if isinstance(payload, dict) else {}
        return "node.setParams", {"nodeId": node_id, **args}
    if path == prefix + "/edges":
        return "graph.connect", payload
    if path == prefix + "/layout/align":
        return "layout.align", payload
    if path == prefix + "/layout/arrange":
        args = dict(payload) if isinstance(payload, dict) else {}
        mode = str(args.pop("mode", "row") or "row").lower()
        command_id = {
            "column": "layout.arrangeColumn",
            "columns": "layout.arrangeColumn",
            "grid": "layout.arrangeGrid",
            "row": "layout.arrangeRow",
            "rows": "layout.arrangeRow",
        }.get(mode, "layout.arrangeRow")
        return command_id, args
    if path == prefix + "/generation/run":
        return "generation.run", payload
    return "", {}


def _opencode_canvas_handle_post(handler, path):
    if not path.startswith(OPENCODE_CANVAS_API_PREFIX + "/"):
        return False
    try:
        payload = _opencode_canvas_parse_json(handler)
    except ValueError:
        _json_err(handler, 413, "Request body too large")
        return True
    except Exception:
        _json_err(handler, 400, "Invalid JSON body")
        return True

    if path == OPENCODE_CANVAS_API_PREFIX + "/runtime/register":
        _json_ok(handler, _opencode_canvas_update_runtime(payload))
        return True
    ...
    command_id, args = _opencode_canvas_command_from_post(path, payload)
    if command_id:
        timeout_seconds = _opencode_canvas_command_timeout_seconds(command_id, args)
        status, response = _opencode_canvas_submit_command(command_id, args, timeout_seconds=timeout_seconds)
...
class Handler(http.server.SimpleHTTPRequestHandler):
    ...
    def send_head(self):
        ...
        if os.path.isdir(path):
            ...
            if parts.path in ("", "/"):
                runtime_index = os.path.join(path, "runtime", "index.html")
                if os.path.isfile(runtime_index):
                    self.send_response(301)
                    self.send_header("Location", "/runtime/")
    ...
    def do_PATCH(self):
        ...
        if not _enforce_local_api_access(self, path):
            return
        if _opencode_canvas_handle_post(self, path):
            return
    ...
    def do_GET(self):
        ...
        if _opencode_canvas_handle_get(self, path):
            return
    ...
    def do_POST(self):
        ...
        if path == "/api/v2/proxy/image":
            ...
            _log_subscription_gate_event(
                ...
                path=self.path,
                workflowId=workflow_id,
                requiredModelId=f"runninghub/{workflow_id}" if workflow_id else "",
                vipWorkflow=workflow_id in VIDEO_VIP_WORKFLOW_IDS,
            )
            _log_runninghub_workflow_event(
                "proxy_request",
                path=self.path,
                apiUrl=api_url,
                workflowId=workflow_id,
                nodeCount=len(data.get("nodeInfoList") or []) if isinstance(data.get("nodeInfoList"), list) else 0,
                nodes=_summarize_runninghub_node_info_list(data.get("nodeInfoList")),
            )
                ...
                for attempt_idx, delay_sec in enumerate(retry_delays):
                    ...
                    try:
                        ...
                        if header_task_id and allow_task_probe_short_circuit:
                            _log_runninghub_workflow_event(
                                "proxy_response_task_id",
                                workflowId=workflow_id,
                                statusCode=resp.status_code,
                                source="header",
                                taskId=header_task_id,
                            )
                            _json_ok(
                                self,
                                {
                        ...
                        _log_runninghub_workflow_event(
                            ...
                            bodyPreview=full_content[:800].decode("utf-8", "ignore"),
                        )
                        if workflow_id and b"NODE_INFO_MISMATCH" in full_content:
                            _log_runninghub_ai_app_demo_nodes(api_key, workflow_id)
                        self.send_response(resp.status_code)
                        self.send_header("Content-Type", "application/json; charset=utf-8")
                        _send_cors_origin_header(self)
                        self.end_headers()
                ...
                for attempt_idx, delay_sec in enumerate(retry_delays):
                    ...
                    try:
                        ...
                        _log_runninghub_workflow_event(
                            ...
                            bytes=len(resp_data),
                            bodyPreview=resp_data[:800].decode("utf-8", "ignore"),
                        )
                        if workflow_id and b"NODE_INFO_MISMATCH" in resp_data:
                            _log_runninghub_ai_app_demo_nodes(api_key, workflow_id)
                        self.send_response(resp.status)
                        self.send_header("Content-Type", "application/json; charset=utf-8")
                        _send_cors_origin_header(self)
