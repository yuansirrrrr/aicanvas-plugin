...
class MediaFileRouteService:
    ...
    def normalize_virtual_local_path(cls, path_value):
        ...
        if decoded.startswith("runtime/"):
            decoded = decoded[len("runtime/") :].lstrip("/")
        parts = []
    ...
    @staticmethod
    def _is_ip_literal(host_value):
        try:
            ipaddress.ip_address(str(host_value or "").strip("[]"))
            return True
        except Exception:
            return False

    @staticmethod
    def _is_proxy_benchmark_ip(ip_str):
        try:
            ip = ipaddress.ip_address(ip_str)
        except Exception:
            return False
    ...
    def _validate_download_host(self, parsed):
        ...
        try:
            ...
            if not allow_private:
                ...
                for info in infos:
                    ip_str = info[4][0]
                    if self._is_proxy_benchmark_ip(ip_str) and not self._is_ip_literal(host):
                        continue
                    if self._is_private_ip(ip_str):
                        return self._json_err(400, f"Blocked private/reserved address: {host} ({ip_str})")
        except Exception:
            return self._json_err(400, f"DNS resolve failed: {host}")
        return None
