...
def get_runninghub_subscription_workflow_ids():
    ...
    for item in SUBSCRIPTION_GATE_MANIFESTS:
        ...
        if not model_id.startswith("runninghub/") or not workflow_id:
            continue
        if (
            model_id in SUBSCRIPTION_GATE_CANONICAL_EXCLUDES
            or workflow_id in SUBSCRIPTION_GATE_CANONICAL_EXCLUDES
            or _runninghub_model_id(workflow_id) in SUBSCRIPTION_GATE_CANONICAL_EXCLUDES
        ):
            continue
